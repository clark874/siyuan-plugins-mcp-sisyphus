...
> **LLM Wiki 分支：** 当前版本为 `v0.9.0-wiki.1`（CLI `v0.4.0-wiki.1`），新增可验证冷启动诊断、严格 CLI 参数契约、写入收据、AV 语义回读、引用安全删除、自动恢复点、文档/子树权限和事务化附件插入。
...
> **当前 LLM Wiki 版本：**`v0.9.0-wiki.1` — 通过真实 MCP initialize、tools/list 与 bootstrap 验证连接；严格写入新增收据和待确认操作防重；破坏性操作自动保护引用并建立恢复点。CLI `v0.4.0-wiki.1` 要求 Node.js 20+。
