...
> **LLM Wiki 分支：** 当前 Fork 的 `v0.8.1-wiki.2`（CLI `v0.3.1-wiki.1`）修复最近更新摘要和右侧近期差异的前端只读请求适配，同时保留 `v0.8.1` 的原生语义搜索桥接修复及带 SHA-256、备份与失败恢复的中央插件更新通道。思源内置 MCP 仍只作为内部扩展总线，外部 Agent 只注册 Sisyphus 单一入口，插件升级无需逐个重装客户端。
...
> **当前 LLM Wiki 版本：**`v0.8.1-wiki.2` — 近期差异界面修复、语义发现桥接、知识化工具层、严格写入和中央更新通道已统一；本地 CLI 为 `v0.3.1-wiki.1`。
