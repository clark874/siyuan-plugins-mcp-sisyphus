...
> **LLM Wiki 分支：** 当前 Fork 的 `v0.8.8-wiki.2`（本地 CLI `v0.3.7-wiki.2`）修复失效 HTTP 会话与 AV 持有块错误语义，阻止未知 AV 读取产生持久文件，并新增有界的 `search.check_anchor` 统一治理 name/alias/scope。思源内置 MCP 仍只作为内部扩展总线，外部 Agent 只注册 Sisyphus 单一入口。
