...
- Sisyphus 自带的 15 个聚合工具、参数约定、笔记本权限和现有 Agent 工作流继续保持兼容；
...
- **低上下文工具设计**：把 100+ 个思源能力收敛为 15 个按 action 路由的聚合工具，详细说明按需读取。
...
“最近更新”是独立于文件树和命名文档时间线的只读视图。它通过思源 SQL 接口分页读取文档根块，并按实际更新时间降序排列；修改深层子文档后，该文档本身会立即进入前列，无需人为改写各级父文档的更新时间。

- 在左侧栏使用独立时钟图标打开，不占用文档时间线入口；
- 支持按年、月、日切换时间轴粒度，逐页加载更早文档；每个分组可稳定展开或折叠；
- 每篇文档以独立卡片显示标题、父级路径、实际修改时间及懒加载的更新分类；
- 只有确有正文变化时才显示变更块与增删行；可确认的标题变化、现有检查点相同、无历史和历史不足均使用独立状态，避免以 `+0/-0` 冒充正文差异；
- 点击条目同时打开文档和右侧 Diff；也可在空白 Diff 面板或文档树右键菜单直接比较任意当前文档；
- Diff 显示修改、新增、删除状态、章节标题路径、修改前后内容与增删行数；状态标签使用独立行，窄窗口自动改为纵向布局；近期差异模式保持只读，不显示回滚入口；
- 修改、重命名、移动、新建和删除文档后，在面板可见时自动刷新；
- 思源历史接口不返回文档修改前的原始目录，历史归档路径不能作为旧目录证据；因此目录或属性更新只能诚实标为“检查点相同/历史不足”，不能虚构精确前后值；
