---
name: siyuan-mcp-cross-project-relation-closure
description: 思源跨项目方法复用关系闭合工作流。用于审计并建立研究项目到公共方法原子的真实复用关系；不用于项目内语义边、name/alias 日常治理或公共方法原子迁移。
...
```text
timeline(action="create_node", name="跨项目关系闭合前恢复点-<date>", scope="document", documentId="<project-document-id>")
```
```text
block(action="append", parentID="<project-document-id>", dataType="markdown", data="**关系：active-reuse。** 本项目实际复用 ((<method-atom-id> 'public-method-name')) 所规定的规则；项目实现差异与适用边界是……。该关系不自动提升项目统计结论的验证状态。")
```
```text
block(action="set_attrs", id="<relation-block-id>", attrs={"custom-relation-kind":"active-reuse"})
```
...
```
```text
search(action="query_sql", stmt="SELECT r.block_id, r.def_block_id, b.root_id, b.hpath FROM refs r JOIN blocks b ON b.id = r.block_id WHERE r.block_id = '<relation-block-id>' AND r.def_block_id = '<method-atom-id>' LIMIT 20", maxRows=20)
```
```text
search(action="get_backlinks", id="<method-atom-id>", mode="both")
```
```text
search(action="knowledge", query="哪些研究项目实际使用了这个方法？", pageSize=10, candidateSize=30)
```
