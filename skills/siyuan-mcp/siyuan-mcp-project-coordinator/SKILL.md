...
```
```text
search(action="query_sql", stmt="SELECT block_id, name, value FROM attributes WHERE block_id IN ('<event-id-1>','<event-id-2>') AND name IN ('custom-progress-workstream','custom-progress-kind','custom-progress-occurred-at','custom-progress-provider','custom-progress-session-id','custom-provenance-occurred-at','custom-provenance-source-provider','custom-provenance-source-session','custom-provenance-compile-provider','custom-provenance-compile-session') ORDER BY block_id, name", maxRows=1000)
```
```text
