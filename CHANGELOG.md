...
- 带状态保护的写入复用同一次预检返回的哈希凭据与 request ID；新增型写入可通过现有 `validateOnly` 领取 ID，不新增 MCP action，也不削弱幂等与新鲜度校验
- 项目协调 Skill v0.6.2 将未登记目录的“收尾”拆为最小中枢、项目源登记、进度页、知识编译和常规收尾五个可回读阶段；首次创建不再把知识原子批量塞进中枢文档
- 普通进度事件只要求稳定事件 ID，不再误用 UUIDv7 承担时间排序；项目事实顺序继续以 `occurredAt` 为唯一机器依据；CLI 提升至 `v0.4.14`

## v0.9.13 - 2026-09-04

- `project.snapshot` 先读取最多 501 个事件，再在 TypeScript 中按 `occurredAt`、登记时间和块 ID 稳定排序；项目头部、工作线头部、投影诊断与本地探针基线共用同一事实时间线
- 快照新增 `chronology` 完整性、扫描数量和头部字段；事件超过 500 条时返回截断诊断并停止权威投影滞后判断，防止不完整窗口驱动状态更新
- `provenance.record_event` 对目标原子的最新溯源摘要采用单调时间门：历史补录保留完整事件与引用，但不覆盖较新摘要；`list_atom_events` 同步按事实时间排序
- 项目协调 Skill v0.6.1 增加当前增量、历史补录、历史冲突、重复内容四分类，固定 `occurredAt` 取证顺序，并禁止历史补录回退当前项目或工作线状态；CLI 提升至 `v0.4.13`
...
- 思源不可用降级：重试一次失败后停止访问共享记忆，仅本地工作并在恢复后引导重新启动与收尾；降级状态下禁止项目写入
- 固定数据边界新增写入护栏：进度与知识写入只经 Sisyphus MCP，禁止经 RepoPrompt 等本地写入通道修改项目文件、进度页或知识原子
- 会话表限额：读取上限 100、普通全景展示上限 20、最近 48 小时 `lastSeenAt` 标注"活跃"；完成门新增启动核验验收项
- 实测：投影一致性探针在试点项目首跑捕获真实投影滞后；Git/find 探针语义验证通过。插件提升至 `v0.9.10`，CLI 提升至 `v0.4.10`，不新增 MCP action

## v0.9.11 - 2026-09-04

- 项目协调 Skill 将启动核验收敛为“本地权威文件—项目源清单—思源投影/知识”三方对账；仅对清单中已有哈希的 Tier A 文件执行内容核验，mtime 与 Git 状态只作为差量线索
- 项目状态按全项目最新事件校验，工作线状态按同一工作线最新事件校验；无事件的工作线不再错误追随全局最新事件
- 事件读取固定为单表 `EXISTS` 取 ID、再单表读取属性，禁止退回多重 `attributes LEFT JOIN + ORDER BY/LIMIT`；普通事件和知识事件分别按 progress/provenance 属性归因会话
- 非 Git 目录统一使用 `YYYY-MM-DD HH:MM:SS UTC`，先检查 `find` 退出状态再截断结果；无普通收尾时将事件时间明确标为弱基线
- 修复插件、CLI、接入包、双语说明与稳定发布通道的版本漂移；插件提升至 `v0.9.11`，CLI 提升至 `v0.4.11`，不新增 MCP action

## v0.9.9 - 2026-09-03

- 项目协调 Skill 新增唯一“项目全景输出契约”：固定项目说明、阶段历程、当前状态、核心观点与发现、权威文件、实质更新、Agent 会话、冲突边界和下一步入口九节顺序
- 新增权威来源优先级与项目归属门；协调器开发、部署及用研究项目作样本的测试不得再进入研究项目进度
- 进度页数据模型增加 `project-profile`、`stage-ledger`、`artifact-index` 三类稳定投影；产物索引只保存相对路径，输出时核验并生成完整绝对路径
- 会话入口禁止截断 sessionId、preferredUrl、launcherUrl 和 resumeCommand；普通全景隐藏内部 ID、SQL、MCP action、捕获方式和无项目贡献的测试会话
- 生成链与 Skills-over-MCP 同步分发输出契约参考文件；插件提升至 `v0.9.9`，CLI 提升至 `v0.4.9`，不新增 MCP action

## v0.9.8 - 2026-09-03

- 项目协调 Skill 改为“启动/交接/知识化/收尾”四个独立命令：启动与交接默认输出完整项目进度全景，知识化只处理长期知识增量，收尾先知识化再登记并说明本次会话的工作差量
- 项目进度全景统一展示项目与工作线状态、完整任务事件时间线、知识变化及验证状态、已登记 Agent 会话、关键产物、阻塞与冲突；收尾额外区分本会话成果与同期其他 Agent 更新
- OpenAI Skill 元数据将协调器的中文可见名称设为“项目协同”，底层英文 Skill 标识保持不变，继续禁止隐式调用
- 插件提升至 `v0.9.8`，CLI 同步提升至 `v0.4.8`；不新增 MCP action，不改变既有单事件原则和进度数据模型
...
- 严格写入协调器将每个已保存搜索名称识别为独立状态目标；保存与删除均基于真实条件仓库生成前置哈希，并在写后核验存在性和条件对象
...
## v0.9.2-wiki.4 - 2026-08-26

- 明确维护分支的产品定位为 LLM Wiki 知识治理网关，不再吸收面向通用笔记软件的图片读取能力；移除 `file.get_image_ocr_text`，并在策略层永久拒绝原生 `image` 工具，即使用户清空持久化屏蔽列表也不会暴露
- `av.render` 兼容思源内核的表格分组行、画廊卡片和组内独立分页：归一化当前页数据，保留去重后的分组元数据，并以最大分组页数计算 `hasNextPage`
- `av.set_cells` 停止写入关系值，避免在无法完整验证目标 AV 权限时形成越界关系；同时拒绝图片型 `mAsset`，普通文本字段和文件型资产保持不变
- 本补丁不引入上游图片直读、图片引用审计、批量导出快照、链接目标补全、AV 视图配置写入或插件包诊断套件；插件提升至 `v0.9.2-wiki.4`，CLI 提升至 `v0.4.2-wiki.4`
...
- 内容通道限定为安全扩展名的 UTF-8 文本，单文件不超过 1 MiB，每次默认返回 8,000 字符、最多 20,000 字符；分页在统一敏感信息脱敏后执行，已识别的密钥值在全文重复位置一并清除，私钥整块清除，普通响应不披露本机绝对路径
- 二进制、敏感、超限、未列入清单或绑定陈旧的文件不返回内容；对可核验文件报告清单收录、可读性、内容读取、当前哈希、清单哈希匹配与修订版本核验状态，避免把“存在”“可读”或“清单已收录”误报为内容已核验
- Git 项目在绑定 `HEAD` 一致之外继续核对目标文件工作区状态；文件存在未提交改动时仍可按只读边界返回脱敏内容，但明确将 `revisionVerified` 置为否
- MCP/CLI 文件 Skill、帮助、设置文案与 Agent Kit 同步增加“登记—扫描—受控读取—必要时解析”工作流；插件提升至 `v0.9.2-wiki.1`，CLI 提升至 `v0.4.2-wiki.1`
- 本版本不增加项目源写入、删除、命令执行、目录级任意读取或 Agent 工作区扩展；既有项目的存量审计仅在真实安装与 MCP 验收完成后按专门手册执行

## v0.9.1-wiki.1 - 2026-08-22

- `file.register_project_source` 建立可移植 `projectId`、思源项目中枢稳定块 ID 与当前主机工作目录绑定；Git 登记核对仓库根、`HEAD` 和远端身份，普通目录采用独立身份，绝对路径只保存在插件私有主机绑定中
- `file.scan_project_manifest` 生成有界 A/B/C 分层清单：A 层仅包含显式指定的核心文件并在单文件与总读取量上限内计算 SHA-256，B 层只保存普通文件元数据，C 层记录缓存、依赖、构建产物、自定义排除项、符号链接与特殊文件；扫描对排除目录提前剪枝并设置条目上限
- `file.resolve_project_source` 拒绝绝对路径、父目录穿越和逃逸工作目录的符号链接，仅返回当前主机路径、存在性、清单收录与版本状态，不读取文件内容；`file.list_project_sources` 分页返回项目与绑定摘要并固定隐藏本机根目录
- 登记与扫描纳入严格写入预检、请求去重、状态回读及协议级确认；路径解析因披露本机绝对路径同样要求确认。项目源登记表保存在插件受控存储中，不进入普通知识搜索结果
- MCP/CLI 文件 Skill 同步新增“登记—扫描—列出—解析”流程，并明确解析成功不等于内容已核验；CLI 升级至 `v0.4.1-wiki.1`
- 本版本不增加任意项目文件内容读取，不把外部目录加入 Agent 工作区，也不执行既有项目的全库审计；受控内容读取与存量项目审计继续归入 `0.9.2`
...
- 精确锚点多命中时失败关闭并返回全部可读候选；仅当 `activeScopes` 与恰好一个 `custom-anchor-scope` 相交时确定性解析，不以语义排名掩盖命名冲突；唯一句中锚点作为显式种子与语义候选合并
- 命名空间结果附带现有验证状态与脱敏来源元数据，并明确“确定性定位不等于证据获准”；`namespaceMode="off"` 仅保留为同版本检索基线，不作为普通 Agent 工作流
- 新增零外部依赖的真实库检索评测：四路比较全文、原生语义、旧知识编排与命名空间优先编排，报告 Hit@1/3/5、MRR、nDCG@5、歧义解析、无答案误命中、时延、响应字节与外部调用；私有稳定块 ID 评测集默认不进入 Git，也不加入默认 CI
- 30 题实机基准中，命名空间优先相对旧知识编排将 Hit@1 从 53.3% 提高到 76.7%、Hit@5 从 60.0% 提高到 80.0%，外部嵌入调用由 30 次降至 17 次；同时如实保留跨语言 Hit@5 仅 40% 与无答案负例误命中率 100% 的当前边界
- 实机验证思源 3.8.1 原生语义索引的新增、更新、删除生命周期：更新后首次查询命中新文本，删除后 `blocks` 与 `block_embeddings` 对应记录均归零，证明无需维护 QMD 第二索引
- Skill、服务初始化说明、中英文工具文档和帮助快照同步采用“命名空间优先、歧义保守、语义兜底”协议；明确不引入 QMD、ranx、Fuse.js、本地模型或第二套索引
- 本地 CLI 同步提升至 `v0.3.9-wiki.1`；推荐思源版本保持 `3.8.1`
...
- 笔记本根树不再把顶层叶子文档计入 `partial`、`failedTopLevelDocumentCount` 或 `errors`；共享读取器先用 `listDocsByPath` 判定直接子文档，并在并发删除后重新核验，真实内核故障继续保持硬错误
- 新增忠实模拟内核无子目录报错的单元测试与真实思源 smoke，覆盖三个显式入口、根树无假警报、父文档后代保留和真实 `api_error` 不软化
- 本地 CLI 元数据同步提升至 `v0.3.8-wiki.2`；推荐思源版本保持 `3.8.1`
...
- 修正 `document.reorder` / `fs.reorder` 的排序作用域：父文档重排在 3.8.1 上只把该父文档切换为手动排序 `6`，不再误改整个笔记本；笔记本根重排与旧版思源继续使用笔记本级兼容路径
- `av.render` 新增 `ignoreRows`，可只读取视图和字段结构而省略行值，降低数据库盘点与 Agent 规划的输出体积；同时兼容思源 3.8.1 的 `view.rows` / `view.columns` 嵌套响应，恢复正确分页与轻量表格摘要，并移除重复的原始行副本
- 严格写入协调器将文档排序声明、实际生效模式和完整子文档次序纳入状态哈希与提交后回读；补齐动作契约、权限分类、旧内核回退和思源 3.8.1 专项回归测试
- 本地 CLI 元数据同步提升至 `v0.3.4-wiki.1`；推荐思源版本更新为 `3.8.1`

## v0.8.4-wiki.1 - 2026-08-19

- 根树组装改用并发上限为 8 的任务池，避免大量一级文档同时触发突发内核请求；单个子树读取失败时保留一级节点并返回 `partial/errors` 与总数、失败数，使真实叶节点和残缺子树可被明确区分
...
- 本地 CLI 元数据同步提升至 `v0.3.3-wiki.1`；npm 公共包不在本轮发布范围内
