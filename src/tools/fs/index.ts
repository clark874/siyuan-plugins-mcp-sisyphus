...
export const FS_VARIANTS: ActionVariant<FsAction>[] = [
    ...
    createZodActionVariant('write', FsWriteSchema, 'Create a document or replace an existing document body when overwrite=true.'),
    createZodActionVariant('replace', FsReplaceSchema, 'Apply exact old/new text replacement edits inside one Markdown document.'),
