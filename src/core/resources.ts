...
function renderToolOverview(): string {
    ...
    return [
        ...
        '',
        'This server exposes 16 aggregated tools. Use `fs` first for basic path-style notebook and document operations; use `project.snapshot` for project-memory recovery.',
        '',
        '## High-risk actions',
        '',
...
function renderWriteSafety(): string {
    return [
        ...
        '2. Read `preconditionField`, the returned temporary credential, and `issuedRequestId`.',
        '3. Execute once with the same business arguments, the returned `issuedRequestId`, and that credential.',
        '4. Re-read the affected target and verify the result.',
        '',
        '## Request-id-only additive mutation',
        '',
        'Actions whose schema has no expected-hash field, such as create or append actions, do not issue a lease. A capable client may execute once with its own fresh UUIDv7. Otherwise call `validateOnly=true`, then execute once with the returned `issuedRequestId`; this preflight issues no hash credential and is not a failure.',
        '',
...
function buildStaticHelpResources(): HelpResourceDefinition[] {
    return [
        ...
        },
        {
            ...
            description: 'Explains server-issued UUIDv7 request IDs and the preflight-lease protocol required for strict mutations.',
            mimeType: MIME_TYPE,
