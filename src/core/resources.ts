...
function renderDocumentPathSemantics(): string {
    return [
        ...
        '## Human-readable path',
        '',
        '- Used by `document(action="create")` and `document(action="resolve", hpath=...)`.',
        '- Example: `/Inbox/Weekly Note`',
        '',
        '## Storage path',
        '',
        '- Used by `document(action="rename")`, `document(action="remove")`, `document(action="move")`, and `document(action="resolve", path=...)` when you pass `notebook + path`.',
        '- Obtain it from `document(action="resolve", id=..., include=["path"])` first.',
        ...
        '2. Reuse the returned storage path for path-based `rename`, `remove`, `move`, or `resolve`.',
