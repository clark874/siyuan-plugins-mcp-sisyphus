...
/**
 * 进程级修改协调器。HTTP 模式下所有会话共享同一个运行时，
 * 因而该互斥器、租约池和账本共同构成 Sisyphus 的唯一写入网关。
 */
export class WriteSafetyCoordinator {
    ...
    private async recordUnknown(
        ...
        try {
            ...
            // 调用方已经收到 outcome_unknown，不应再用次生账本持久化失败覆盖它。
        }
    }
...
async function probePostWriteState(
    ...
    // 思源部分修改会在二级索引和派生文档树更新前先返回成功。
    // 有界回读轮询可避免把真实提交误判为 no_change，
    // 同时让合法幂等或无变化写入及时结束。
...
async function appendHumanPathState(
    ...
    for (const path of physicalPaths) {
        ...
        // blocks SQL 表异步建索引；若据此计算哈希，block.append 后的即时写入
        // 可能复用陈旧哈希。这里改读实时块树与各块当前 kramdown，
        // 使预检观察到与实际写入处理器一致的编辑器状态。
