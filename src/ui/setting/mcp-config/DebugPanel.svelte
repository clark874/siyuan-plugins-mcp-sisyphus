<script lang="ts">
  ...
  function buildDebugItems(
    ...
    return [
      ...
      {
        type: "checkbox",
        key: "writeSafety__referenceProtection",
        value: currentConfig.writeSafety.referenceProtection,
        title: label(
          "write_safety_reference_title",
          "Protect Referenced Blocks",
        ),
        description: label(
          "write_safety_reference_desc",
          "Before ID-destroying mutations, inspect external block references and reject by default until referencePolicy=break is explicitly approved.",
        ),
      },
      {
        type: "select",
        key: "writeSafety__autoRecovery",
        value: currentConfig.writeSafety.autoRecovery,
        title: label(
          "write_safety_recovery_title",
          "Automatic Recovery Points",
        ),
        description: label(
          "write_safety_recovery_desc",
          "Create a timeline recovery point before destructive writes. Required mode blocks the mutation when a recovery point cannot be created.",
        ),
        options: {
          off: label("write_safety_recovery_off", "Off"),
          best_effort: label(
            "write_safety_recovery_best_effort",
            "Best effort",
          ),
          required_for_destructive: label(
            "write_safety_recovery_required",
            "Required for destructive writes",
          ),
        },
      },
      {
      ...
      },
      {
        ...
        title: label(
          "version_control_enabled_title",
          "Enable Document Snapshots and Diff",
        ),
        description: label(
          "version_control_enabled_desc",
          "Register the left snapshot dock, right diff dock, command, and editor listeners. Turn this off to remove both docks.",
        ),
      },
      {
        type: "checkbox",
        ...
        title: label(
          "recent_documents_enabled_title",
          "Enable Recently Modified Dock",
        ),
        description: label(
          "recent_documents_enabled_desc",
          "Register an independent dock that lists documents by their actual modified time.",
        ),
      },
      {
        type: "checkbox",
        ...
        description: label(
          "debug_slimResponses_desc",
          "Return only the data an agent usually needs. Turn this off to inspect full debug fields, pagination internals, UI refresh metadata, and raw helper metadata.",
        ),
      },
      {
        type: "checkbox",
        ...
        title: label(
          "version_control_show_debug_meta_title",
          "Timeline Debug Metadata",
        ),
        description: label(
          "version_control_show_debug_meta_desc",
          "Show document/block IDs and raw diff statuses in the snapshot and diff docks.",
        ),
      },
      {
        type: "checkbox",
        ...
        description: label(
          "puppy_testMode_desc",
          "Randomly cycle real MCP actions for animation testing without calling tools.",
        ),
        layout: "inline",
        children: [
            ...
            ? [
                {
                  ...
                  description: label(
                    "puppy_testMode_interval_desc",
                    "Delay between random test actions.",
                  ),
                  inputCompact: true,
                  unit: "ms",
                },
...
<div class="debug-settings" class:fn__none={!display}>
    ...
    <div class="debug-section__heading">
      <span class="debug-section__icon" aria-hidden="true">
        <svg viewBox="0 0 24 24">
          <path
            d="M9.5 3.5h5v2h-5v-2ZM7 6.5h10v2h2.5v2H17v3h2.5v2H17v1.25A4.25 4.25 0 0 1 12.75 21h-1.5A4.25 4.25 0 0 1 7 16.75V15.5H4.5v-2H7v-3H4.5v-2H7v-2Zm2 2v8.25A2.25 2.25 0 0 0 11.25 19h1.5A2.25 2.25 0 0 0 15 16.75V8.5H9Z"
          />
        </svg>
      </span>
      <div>
        <h3>
          {getLabel("debug_runtime_section", "Safety settings and diagnostics")}
        </h3>
        <p>
          {getLabel(
            "debug_runtime_section_desc",
            "Control strict safe writes, timeline registration, response detail, and developer metadata. Changes are saved immediately.",
          )}
        </p>
      </div>
    </div>
  ...
  <section class="debug-section">
    <div class="debug-section__heading">
      <span
        class="debug-section__icon debug-section__icon--test"
        aria-hidden="true"
      >
        <svg viewBox="0 0 24 24">
          <path
            d="M10 2h4v2h-1v3.1l5.75 9.96A3.3 3.3 0 0 1 15.9 22H8.1a3.3 3.3 0 0 1-2.85-4.94L11 7.1V4h-1V2Zm.54 9-3.56 6.06A1.3 1.3 0 0 0 8.1 19h7.8a1.3 1.3 0 0 0 1.12-1.94L13.46 11h-2.92Z"
          />
        </svg>
      </span>
      <div>
        ...
        <p>
          {getLabel(
            "debug_test_section_desc",
            "Preview mascot action states without invoking MCP tools.",
          )}
        </p>
      </div>
    </div>
...
<style>
  ...
  .debug-section__icon {
    align-items: center;
    background: var(
      --mcp-config-primary-soft,
      color-mix(in srgb, var(--b3-theme-primary) 12%, transparent)
    );
    border: 1px solid
      var(
        --mcp-config-primary-border,
        color-mix(in srgb, var(--b3-theme-primary) 26%, transparent)
      );
  ...
  .debug-section__icon--test {
    background: color-mix(
      in srgb,
      var(--b3-theme-warning, #d99a24) 10%,
      transparent
    );
    border-color: color-mix(
      in srgb,
      var(--b3-theme-warning, #d99a24) 24%,
      transparent
    );
