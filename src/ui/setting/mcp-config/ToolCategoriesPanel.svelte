    ...
    const ACTION_METADATA: GroupDefinition[] = [
        ...
        },
        {
            ...
            actions: [
                ...
                { key: "search_bazaar", title: "Search Bazaar", description: "Search downloadable SiYuan packages with filters, sorting, and pagination." },
                { key: "get_bazaar_package", title: "Get Bazaar Package", description: "Read exact online package metadata together with local installation state." },
                { key: "read_bazaar_readme", title: "Read Bazaar README", description: "Read one marketplace README as sanitized, redacted, size-limited plain text." },
                { key: "get_plugin", title: "Get Plugin", description: "Inspect one installed plugin and its controlled storage mapping." },
                { key: "list_plugin_updates", title: "List Plugin Updates", description: "List installed plugins reported as outdated without changing them." },
                { key: "list_snippets", title: "List Snippets", description: "List CSS and JavaScript snippets with hashes; content is excluded by default." },
                { key: "list_plugin_storage", title: "List Plugin Storage", description: "List one installed plugin storage root under strict path and symlink limits." },
                { key: "read_plugin_storage", title: "Read Plugin Storage", description: "Read a safe text configuration with size limits and secret redaction." },
                { key: "inspect_plugin", title: "Inspect Plugin", description: "Interpret safe plugin configuration while preserving uncertainty and unknown fields." },
                { key: "plan_change", title: "Plan Change", description: "Create an expiring reversible plan with a target-state hash and redacted diff." },
                { key: "apply_change", title: "Apply Change", description: "Apply and verify one non-stale plan with automatic recovery on failure." },
                { key: "rollback_change", title: "Rollback Change", description: "Restore and verify the exact pre-change state for an applied change." },
                { key: "discard_change_plan", title: "Discard Plan", description: "Discard an unapplied or expired plan without changing its target." },
                { key: "list_control_changes", title: "List Control Changes", description: "List compact redacted control-plane plans and audit records." },
                { key: "get_control_change", title: "Get Control Change", description: "Read one compact redacted plan or change record." },
            ],
        },
        {
            category: "flashcard",
