<script lang="ts">
  ...
  let versionControlSettings: VersionControlSettings =
    buildDefaultVersionControlSettings();
  let permissionDisplaySettings: PermissionDisplaySettings =
    buildDefaultPermissionDisplaySettings();
  let focusGroup = "";
  let lastFocusGroup = "";
  ...
  $: userRulesGroupLabel = getLabel(
    USER_RULES_GROUP_KEY,
    USER_RULES_GROUP_LABEL,
  );
  ...
  $: tabItems = [
    {
      id: HTTP_GROUP_KEY,
      label: httpGroupLabel,
      description: getLabel(
        "settingsConnectionDesc",
        "Connect MCP clients or use the standalone CLI, then inspect the current service status.",
      ),
      iconSvg: ICON_SVGS.globe,
    },
    {
      id: PERM_GROUP_KEY,
      label: permGroupLabel,
      description: getLabel(
        "settingsPermissionsDesc",
        "Control which notebooks MCP clients can read, edit, or delete.",
      ),
      iconSvg: ICON_SVGS.lock,
    },
    {
      id: TOOL_GROUP_KEY,
      ...
      description: getLabel(
        "settingsToolsDesc",
        "Choose the grouped tools and actions exposed to connected agents.",
      ),
      iconSvg: ICON_SVGS.folder,
    },
    {
      id: MCP_APPS_GROUP_KEY,
      label: mcpAppsGroupLabel,
      description: getLabel(
        "settingsMcpAppsDesc",
        "Control interactive MCP Apps and the actions performed manually inside them.",
      ),
      iconSvg: ICON_SVGS.layout,
    },
    {
      id: PUPPY_GROUP_KEY,
      label: puppyGroupLabel,
      description: getLabel(
        "settingsMascotDesc",
        "Adjust the on-screen mascot behavior and preview its appearance.",
      ),
      iconSvg: ICON_SVGS.paw,
    },
    {
      id: ANALYTICS_GROUP_KEY,
      label: analyticsGroupLabel,
      description: getLabel(
        "settingsAnalyticsDesc",
        "Review local usage patterns, activity trends, and tool statistics.",
      ),
      iconSvg: ICON_SVGS.barChart,
    },
    {
      id: DEBUG_GROUP_KEY,
      label: debugGroupLabel,
      description: getLabel(
        "settingsDebugDesc",
        "Manage advanced diagnostics and developer-oriented display options.",
      ),
      iconSvg: ICON_SVGS.bug,
    },
    {
      id: USER_RULES_GROUP_KEY,
      label: userRulesGroupLabel,
      description: getLabel(
        "settingsRulesDesc",
        "Add durable instructions and workspace memory for connected agents.",
      ),
      iconSvg: ICON_SVGS.compass,
    },
    {
      id: FEEDBACK_GROUP_KEY,
      label: feedbackGroupLabel,
      description: getLabel(
        "settingsFeedbackDesc",
        "Send problems, suggestions, or product experience directly to the developer.",
      ),
  ...
  onMount(async () => {
    ...
    permissionDisplaySettings =
      await loadPersistedPermissionDisplaySettings(plugin);
    ...
    if (savedPerms && typeof savedPerms === "object") {
      const normalizedPermissions = Object.fromEntries(
        Object.entries(savedPerms).map(([notebookId, permission]) => [
          notebookId,
          normalizeNotebookPermission(permission),
        ]),
      );
      permissions = normalizedPermissions;
      if (
        JSON.stringify(savedPerms) !== JSON.stringify(normalizedPermissions)
      ) {
  ...
  function setMcpAppEnabled(
    appName: keyof ToolConfig["mcpApps"],
    enabled: boolean,
  ) {
    config = {
      ...config,
      mcpApps: {
        ...config.mcpApps,
        [appName]: { ...config.mcpApps[appName], enabled },
      },
    };
  }

  function setMcpAppActionEnabled(
    appName: keyof ToolConfig["mcpApps"],
    action: string,
    enabled: boolean,
  ) {
    ...
    config = {
      ...
      mcpApps: {
        ...
        [appName]: {
          ...appConfig,
          enabled: enabled ? true : appConfig.enabled,
          actions: { ...appConfig.actions, [action]: enabled },
        },
      },
    };
  }
  ...
  async function persistPermissionDisplaySettings() {
    if (plugin) {
      permissionDisplaySettings = await savePersistedPermissionDisplaySettings(
        permissionDisplaySettings,
        plugin,
      );
  ...
  async function persistTelemetryConfig() {
    if (plugin) {
      telemetryConfig = await savePersistedTelemetryConfig(
        telemetryConfig,
        plugin,
      );
    }
  ...
  async function persistVersionControlSettings() {
    if (plugin) {
      versionControlSettings = await savePersistedVersionControlSettings(
  ...
  const onChanged = async (event: CustomEvent<ChangeEvent>) => {
    ...
    if (key === "puppy__testModeIntervalMs") {
      ...
      puppySettings = {
        ...puppySettings,
        testModeIntervalMs: Number.isFinite(numeric)
          ? Math.max(800, Math.min(10000, Math.floor(numeric)))
          : puppySettings.testModeIntervalMs,
      };
      await persistPuppySettings();
      return;
    }
    ...
    if (key.startsWith("perm__") && key !== "perm__hint") {
      ...
      permissions = {
        ...permissions,
        [notebookId]: value as NotebookPermission,
      };
    ...
    if (key === "writeSafety__strictMode") {
      ...
      try {
        const restarted =
          await plugin?.refreshHttpServerAfterInstructionConfigChange?.();
        showMessage(
          getLabel(
            restarted
              ? "write_safety_restarted"
              : "write_safety_saved_reconnect",
            restarted
    ...
    if (key === "writeSafety__referenceProtection") {
      config = {
        ...config,
        writeSafety: {
          ...config.writeSafety,
          referenceProtection: Boolean(value),
        },
      };
      await persistConfig();
      return;
    ...
    if (key === "writeSafety__autoRecovery") {
      const autoRecovery = String(value);
      if (
        autoRecovery === "off" ||
        autoRecovery === "best_effort" ||
        autoRecovery === "required_for_destructive"
      ) {
        config = {
          ...config,
          writeSafety: { ...config.writeSafety, autoRecovery },
        };
        await persistConfig();
      }
      return;
    ...
    if (key.startsWith("extension__tool__")) {
      const toolName = decodeURIComponent(
        key.slice("extension__tool__".length),
      );
    ...
    if (key === "file__setting__uploadLargeFileThresholdMB") {
      ...
      config = {
        ...
        file: {
          ...config.file,
          uploadLargeFileThresholdMB: Number.isFinite(numeric)
            ? Math.max(1, Math.min(1024, Math.floor(numeric)))
            : config.file.uploadLargeFileThresholdMB,
        },
      };
      await persistConfig();
      return;
    ...
    if (key === "userRulesText" || key === "agentSiyuanMemoryText") {
      ...
      config = {
        ...
        ...(key === "agentSiyuanMemoryText"
          ? {
              agentSiyuanMemoryUpdatedAt: nextText.trim()
      ...
      await persistConfig();
      try {
        const refreshInstructionConfig =
          plugin?.refreshHttpServerAfterInstructionConfigChange ??
          plugin?.refreshHttpServerAfterUserRulesChange;
        ...
        showMessage(
          restarted
            ? getLabel(
                isAgentMemoryChange
                  ? "agent_memory_http_restarted"
                  : "user_rules_http_restarted",
                isAgentMemoryChange
            ...
            : getLabel(
                isAgentMemoryChange
                  ? "agent_memory_saved_reconnect"
                  : "user_rules_saved_reconnect",
                isAgentMemoryChange
        ...
        showMessage(
          getLabel(
            isAgentMemoryChange
              ? "agent_memory_refresh_failed"
              : "user_rules_refresh_failed",
            isAgentMemoryChange
    ...
    if (key === "telemetry__interval") {
      ...
      telemetryConfig = {
        ...telemetryConfig,
        reportIntervalHours: Number.isFinite(hours)
          ? Math.max(1, Math.min(168, hours))
          : telemetryConfig.reportIntervalHours,
      };
      await persistTelemetryConfig();
      return;
    }
    ...
    if (key.startsWith("mcpApps__")) {
      ...
      if (appName && kind === "enabled")
        setMcpAppEnabled(
          appName as keyof ToolConfig["mcpApps"],
          Boolean(value),
        );
      else if (appName && kind === "action" && action)
        setMcpAppActionEnabled(
          appName as keyof ToolConfig["mcpApps"],
          action,
          Boolean(value),
        );
  ...
  export async function saveSettings() {
    ...
    await persistConfig();
    showMessage(
      plugin?.i18n?.mcpConfigSaved || "✅ MCP Tools configuration saved",
    );
  }
  ...
  export async function resetDefaults() {
    ...
    await persistPermissionDisplaySettings();
    showMessage(
      plugin?.i18n?.mcpConfigReset ||
        "🔄 MCP Tools configuration reset to defaults",
    );
  }
</script>
...
<div class="fn__flex-1 fn__flex config__panel">
  <aside class="config__sidebar">
    ...
    <nav
      class="config__navigation"
      aria-label={getLabel("settingsNavigationLabel", "Settings navigation")}
    >
      {#each tabItems as tab}
        ...
        >
          <span class="config__nav-icon" aria-hidden="true"
            >{@html tab.iconSvg}</span
          >
      ...
      <div class="config__tab-content">
        {#if activeTab}
          <header class="config__page-header">
            <span class="config__page-icon" aria-hidden="true"
              >{@html activeTab.iconSvg}</span
            >
            <div>
        ...
        <HttpServerPanel
          {plugin}
          group={httpGroupLabel}
          display={focusGroup === HTTP_GROUP_KEY}
          bind:httpSettings
          {getLabel}
        />
        <PermissionsPanel
          group={permGroupLabel}
          display={focusGroup === PERM_GROUP_KEY}
          {notebooks}
          {permissions}
          {permissionDisplaySettings}
          {permLoading}
        ...
        />
        <McpAppsPanel
          display={focusGroup === MCP_APPS_GROUP_KEY}
          {config}
          {getLabel}
          {onChanged}
        />
        <PuppyPanel
          group={puppyGroupLabel}
          display={focusGroup === PUPPY_GROUP_KEY}
          {puppySettings}
          {getLabel}
          {onChanged}
        />
        ...
        />
        <DebugPanel
          group={debugGroupLabel}
          display={focusGroup === DEBUG_GROUP_KEY}
          {config}
          {puppySettings}
          {versionControlSettings}
          {getLabel}
        ...
        />
        <UserRulesPanel
          group={userRulesGroupLabel}
          display={focusGroup === USER_RULES_GROUP_KEY}
          {config}
          {getLabel}
          {onChanged}
        />
        <FeedbackPanel
          group={feedbackGroupLabel}
          display={focusGroup === FEEDBACK_GROUP_KEY}
          {plugin}
          {getLabel}
        />
      </div>
...
<style lang="scss">
  .config__panel {
    ...
    --mcp-config-caption-color: var(
      --b3-theme-on-surface-light,
      var(--b3-theme-on-surface)
    );
    --mcp-config-code-font: var(
      --b3-font-family-code,
      ui-monospace,
      SFMono-Regular,
      Menlo,
    ...
    --mcp-config-surface: color-mix(
      in srgb,
      var(--b3-theme-surface) 91%,
      var(--b3-theme-background)
    );
    --mcp-config-surface-raised: color-mix(
      in srgb,
      var(--b3-theme-surface) 97%,
      var(--b3-theme-background)
    );
    --mcp-config-border: color-mix(
      in srgb,
      var(--b3-border-color) 88%,
      transparent
    );
    --mcp-config-primary-soft: color-mix(
      in srgb,
      var(--b3-theme-primary) 13%,
      transparent
    );
    --mcp-config-primary-border: color-mix(
      in srgb,
      var(--b3-theme-primary) 28%,
      transparent
    );
    --mcp-config-surface-accent: linear-gradient(
        135deg,
        var(--mcp-config-primary-soft),
        transparent 70%
      ),
      var(--mcp-config-surface-raised);
    --mcp-config-shadow: 0 1px 2px
        color-mix(in srgb, var(--b3-theme-on-background) 7%, transparent),
      0 8px 24px
        color-mix(in srgb, var(--b3-theme-on-background) 3%, transparent);
  ...
  .config__tab-content :global(.b3-select:focus) {
    border-color: color-mix(
      in srgb,
      var(--b3-theme-primary) 66%,
      var(--b3-border-color)
    );
    box-shadow: 0 0 0 3px
      color-mix(in srgb, var(--b3-theme-primary) 11%, transparent);
    outline: none;
  }
