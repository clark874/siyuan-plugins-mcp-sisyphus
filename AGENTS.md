...
两种接口共享同一套底层能力（15 个聚合工具、163 个注册 action，不含各工具的 `help` 与动态 `extension` action；`extension` 另桥接经过固定白名单筛选的官方原生 MCP 工具）。
...
- 作者：Taihong Yang
- 许可证：MIT
- 当前版本：`0.9.11`（根 `package.json`、`plugin.json` 与接入包装器同步；CLI 子包版本独立管理，当前为 `0.4.11`）
