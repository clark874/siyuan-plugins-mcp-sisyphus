...
> **Local maintenance version:** `v0.2.4-local.2` adds the knowledge-ingestion Skill and the refined recent-history comparison states shared with the plugin. The latest upstream npm release remains `v0.2.3`; this local prerelease must be published explicitly before npm installation can provide these updates.
