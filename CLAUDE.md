...
仓库现在同时发布两种产物：SiYuan 插件（根 `package.json` + `package.zip`）和独立 CLI（`cli/` 子目录，发布到 npm 包 `siyuan-mcp`，安装后提供 `siyuan` 命令）。

**CLI 定位**：类 `obsidian-cli` 的直接操作工具 —— `siyuan <tool> <action> [--flag value ...]`，比如：
```bash
siyuan notebook list
siyuan block append --parent-id <id> --data-type markdown --data "..."
siyuan document list-tree --notebook <id> --json | jq '.data[].hPath'
```
...
- `src/cli/` —— CLI 源码：
  ...
  - `config.ts` 读写 `~/.siyuan-mcp/config.json`（只有 `apiUrl` + `token`）
  - `init.ts` 精简版交互初始化
- `cli/` —— 独立 npm 子包（`package.json`、`README.md`、自包含产物 `dist/cli.cjs`）
- `~/.siyuan-mcp/config.json` —— 用户侧默认配置路径（`init` 命令生成，权限 `0600`）

### 构建 & 发布
```bash
pnpm build:cli     # 产出 cli/dist/cli.cjs（自包含 CJS bundle）
pnpm publish:cli   # 先构建再发布 siyuan-mcp 包（提供 siyuan 命令）
```
...
- 配置优先级：CLI flag > 环境变量 > `~/.siyuan-mcp/config.json` > 默认值。
