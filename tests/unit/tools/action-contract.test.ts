...
describe('tool action contract coverage', () => {
    ...
    it('covers every document action with a minimal endpoint contract', async () => {
        await runContracts('document', DOCUMENT_VARIANTS, callDocumentTool as ToolCaller, [
            ...
            // A notebook root is assembled from listDocsByPath instead, so the
            // listDocTree contract is pinned with a document storage path.
            { action: 'list_tree', args: { action: 'list_tree', notebook: 'nb-1', path: '/doc-1.sy' }, expectedEndpoint: '/api/filetree/listDocTree' },
    ...
    it('covers every system action with a minimal endpoint contract', async () => {
        await runContracts('system', SYSTEM_VARIANTS, callSystemTool as ToolCaller, [
            ...
            { action: 'read_bazaar_readme', args: { action: 'read_bazaar_readme', kind: 'plugin', packageName: 'demo-plugin' }, expectedEndpoint: '/api/bazaar/getBazaarPlugin', expectedError: true },
