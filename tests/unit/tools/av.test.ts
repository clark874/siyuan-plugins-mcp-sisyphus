...
describe('av tool', () => {
    ...
    it('inspects an AV definition and reports owning block/document candidates without deleting', async () => {
        const avApi = await import('@/api/av');
        const searchApi = await import('@/api/search');
        const context = await import('@/tools/internal/context');
        const transactionApi = await import('@/api/transaction');
        vi.mocked(avApi.getAttributeView).mockResolvedValue({
            av: {
                ...
                keyValues: [{
                    key: { id: 'primary', type: 'block' },
                    values: [{ id: 'value-1', blockID: 'row-1', block: { id: 'source-1' } }],
                }],
            },
        });
        vi.mocked(avApi.getMirrorDatabaseBlocks).mockResolvedValue({ refDefs: [{ refID: 'db-block-1' }] });
        vi.mocked(searchApi.querySQL).mockResolvedValue([{ id: 'db-block-1' }]);
        vi.mocked(context.resolveDocumentContextById).mockResolvedValue({
            documentId: 'doc-1', notebook: 'nb-1', path: '/doc-1.sy',
        });

        const result = await callAvTool(client, {
            action: 'inspect',
            avID: 'av-1',
        }, enabledActions('inspect'), permMgr);

        expect(JSON.parse(result.content[0].text)).toMatchObject({
            readonly: true,
            avID: 'av-1',
            definition: { id: 'av-1', name: 'Projects' },
            owningBlockCandidates: ['db-block-1'],
            owningDocumentCandidates: [{ blockID: 'db-block-1', documentID: 'doc-1', notebookID: 'nb-1' }],
            rowCount: 1,
            viewCount: 2,
