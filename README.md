...
> **LLM Wiki 分支：** 当前 Fork 的 `v0.8.8-wiki.1`（本地 CLI `v0.3.7-wiki.1`）修复失效 HTTP 会话与 AV 持有块错误语义，阻止未知 AV 读取产生持久文件，并新增 `search.check_anchor` 统一治理 name/alias/scope。外部 Agent 仍只注册 Sisyphus 单一入口。
