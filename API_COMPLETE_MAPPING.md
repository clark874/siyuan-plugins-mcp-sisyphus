...
> 本表的 459 端点扫描与 119 端点统计来自上游 `v0.6.0` 基线，尚未重新生成全量计数。本地维护版 `v0.6.2-local.5` 另行接入插件清单与状态、在线集市目录与 README、在线更新元数据、代码片段、受限插件存储、允许的系统设置、归档与回滚接口；准确能力边界以 [system 工具文档](docs/reference/tools/system.md) 为准。避免在未重新运行全量映射生成器前手工改写总覆盖率。
